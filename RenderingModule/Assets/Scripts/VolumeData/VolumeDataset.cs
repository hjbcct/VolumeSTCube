    ...
    public class VolumeDataset : ScriptableObject, ISerializationCallbackReceiver
    {
        ...
        private async Task<Texture3D> CreateGradientTextureInternalAsync(IProgressHandler progressHandler)
        {
            ...
            catch (OutOfMemoryException)
            {
                ...
                //{
                //    progressHandler.ReportProgress(x * 2, dimX / 2, "Calculating gradients for slice");
                //    for (int y = 0; y < dimY / 2; y++)
                //    {
                //        for (int z = 0; z < dimZ / 2; z++)
                //        {
                //            int iData = x * 2 + y * 2 * dimX + z * (dimX * dimY);
                //            Vector3 grad = GetGrad(x * 2, y * 2, z * 2, minValue, maxRange);
            ...
            await Task.Run(() => {
                for (int z = 0; z < dimZ; z++)
                {
                    ...
                    //    for (int x = 0; x < dimX / 2; x++)
                    //    {
                    //        int iData = x + y * dimX * 2 + z * 2 * (dimX * dimY);
                    //        Vector3 grad = GetGrad(x * 2, y * 2, z * 2, minValue, maxRange);
